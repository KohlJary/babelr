...
export async function cleanDb(db: ReturnType<typeof createDb>) {
  ...
  await db.execute(sql`TRUNCATE sessions, activities, delivery_queue, collection_items, read_positions, reactions, notification_preferences, invites, objects, actors CASCADE`);
}
